...
The repository defaults in this workspace aim for a solo-maintainer-friendly baseline: Dependabot security features, default-branch rulesets, linear history, and non-fast-forward protection stay enabled, while merge review requirements default to off so a single owner can keep working without needing a second approver. For public repositories, Terraform also enables secret scanning and push protection through the `security_and_analysis` block. For private or internal repositories, secret scanning and push protection depend on GitHub Secret Protection plan and organization settings; verify those GitHub-side controls before relying on them. The default branch ruleset also requires the repository's `test-code`, `test-repo`, and `scan-repo` GitHub Actions checks before merge. A separate release tag ruleset protects `refs/tags/v*` tags from deletion or retagging, and the release workflow also verifies that a tag points at default-branch history before publishing artifacts. `archive_on_destroy` defaults to `true` so a Terraform destroy archives the repository instead of deleting it.
