...
Before `make infra`, update `terraform.tfvars.example` with your repository details. `default_branch = null` leaves an existing repository's current default branch unchanged; set a branch name only when you intentionally want Terraform to manage that setting. Copy `project.env.example` to `project.env` before relying on the shorter default `make infra` path. Before `APPLY=true make infra`, also export a token:
